...
import { CarReasonForm } from './carreason.form';

@Component({
  selector: 'app-carreason',
  templateUrl: './carreason.component.html',
  styleUrls: ['./carreason.component.scss']
})
export class CarreasonComponent implements OnInit {
  token;
  currentUser: any;
  carReason;
  itemId;
  ...
  ngOnInit(): void {
    this.datasource = [];
    const carForm = new CarReasonForm();
    this.carReasonFormGroup = this.formBuilder.group(
      carForm.CarReasonFormBuilder
    );
    const itemForm = new CarReasonForm();
    this.itemFormGroup = this.formBuilder.group(
      itemForm.CarReasonFormBuilder
    );
    this.cols = [
      { field: 'number', header: '#', sortable: true },
      { field: 'name', header: 'เหตุผล', sortable: true },
      { field: 'detail', header: 'รายละเอียด', sortable: true },
      { field: 'action', header: 'Action', sortable: true }
    ];
    this.getCarReasonList();
  }
  ...
  private getCarReasonList() {
    this.spinner.show();
    this.carReason = this.carReason == null ? '' : this.carReason;
    this.carReasonFormGroup.controls['method'].setValue('search');
    this.carReasonFormGroup.controls['name'].setValue(this.carReason);
    this.carReasonFormGroup.controls['user_id'].setValue(this.token);
    this.bookingcarService.ApiCarReason(this.carReasonFormGroup.getRawValue()).subscribe(
      (data) => {
        this.datasource = [];
        if (data) {
          let i = 0;
  ...
  showModalDialogDetail(id, reason, desc) {
    this.spinner.show();
    if (id == 'ADD') {
      this.itemId = null;
      const itemForm = new CarReasonForm();
      ...
      this.itemFormGroup.controls['name'].setValue(reason);
  ...
  saveItem() {
    if (this.getFormValidationErrors()) {
      ...
      if (this.itemFormGroup.controls['id'].value == '0') {
        ...
        this.bookingcarService.ApiCarReason(this.itemFormGroup.getRawValue()).subscribe(
          (data) => {
            if (data.status == 'S') {
