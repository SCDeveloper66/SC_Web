...
export class CartypeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.datasource = [];
    const carForm = new CarTypeForm();
    this.carTypeFormGroup = this.formBuilder.group(
      carForm.CarTypeFormBuilder
    );
    const itemForm = new CarTypeForm();
    this.itemFormGroup = this.formBuilder.group(
      itemForm.CarTypeFormBuilder
    );
    this.cols = [
      { field: 'number', header: '#', sortable: true },
      { field: 'name', header: 'ประเภทรถ', sortable: true },
  ...
  private getCarTypeList() {
    ...
    this.bookingcarService.ApiCartype(this.carTypeFormGroup.getRawValue()).subscribe(
      (data) => {
        this.datasource = [];
        if (data) {
          let i = 0;
  ...
  saveItem() {
    if (this.getFormValidationErrors()) {
      ...
      if (this.itemFormGroup.controls['id'].value == '0') {
        ...
        this.bookingcarService.ApiCartype(this.itemFormGroup.getRawValue()).subscribe(
          (data) => {
            if (data.status == 'S') {
